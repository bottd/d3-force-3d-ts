...
it("forceCenter repositions nodes", () => {
  ...
  f.nodes([a, b, c]);
  f.tick();
  assertNodeEqual(a, { index: 0, x: -100, y: 0, z: 0, vz: 0, vy: 0, vx: 0 });
  assertNodeEqual(b, { index: 1, x: 0, y: 0, z: 0, vz: 0, vy: 0, vx: 0 });
  assertNodeEqual(c, { index: 2, x: 100, y: 0, z: 0, vz: 0, vy: 0, vx: 0 });
});

it("forceCenter respects fixed positions", () => {
  const center = forceCenter();
  const f = forceSimulation().force("center", center).stop();
  const a = { fx: 0, fy: 0, fz: 0 },
    b = {},
    c = {};
  f.nodes([a, b, c]);
  f.tick();
